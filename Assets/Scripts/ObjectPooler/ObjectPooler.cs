...
public class ObjectPooler : AbstractSingleton<ObjectPooler>
{
    ...
    {
        base.Awake(); // gestisce il singleton

        if (Instance != this) // se non è l'istanza attiva, esci
        {
            Destroy(this);
            Debug.LogWarning($"Deleting {this.name} singleton!");
            return;
        }
